                                                        ...
                                                        implements amCoreServices.am_coreservices.AE_PlaylistItem_StatusRequestToPlay_Media
  {
                                     ...
                                     context: amGeneralContext.am_general.A_Context, callback): void
    {
      ...
      if (     (playlistItemType  == amPlaylistItemType.am_coreservices.AE_PlaylistItemType.PlaylistItemType_Image) 
            ...
            //|| (playlistItemType  == amPlaylistItemType.am_coreservices.AE_PlaylistItemType.PlaylistItemType_VideoStream) 
      ...
      //---------------------------------
      self._transitionTo_StatusPlay.getStatus().passToMe(event, eventQueue, error, aPlaylistController, aRenderingController, context, callback);
      var targetZoneId = self.getDiagram().getTargetRenderingZoneId();
      var prepareParams = self.getDiagram().getOwner().getPrepareParams();

      var ctx = self._aSrvLocator._iEntityCreation.createDefaultContext();
      var err = self._aSrvLocator._iEntityCreation.createDefaultError();
      var callbackPrepare2 = function callbackPrepare2()
      {
        ...
        // add item to last Items Played
        const paramIdentifier: amRenderParametersIdentification.am_renderingservices.AE_RenderParams_Identification = prepareParams.getRenderParams().getParamIdentification();
        if (paramIdentifier.getStreamId() !== 0 && paramIdentifier.getId() !== 0 && paramIdentifier.getName() !== "") {
          aPlaylistController._iPlaylistControllerMonitoring.sendLastPlayedItem(paramIdentifier);  
        }
      }
