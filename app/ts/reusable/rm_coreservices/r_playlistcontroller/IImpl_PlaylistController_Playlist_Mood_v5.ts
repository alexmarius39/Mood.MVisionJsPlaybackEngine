...
export module rm_coreservices
{
  export class IImpl_PlaylistController_Playlist_Mood_v5 implements  amCoreServicesIPlaylistControllerPlaylistMoodv5.am_coreservices.I_PlaylistController_Playlist_Mood_v5
  {
                                 ...
                                 error: amGeneralError.am_general.A_Error, context: amGeneralContext.am_general.A_Context, callback)
    {
      ...
      crtPlaylistItemScreenSaverVideoLogic.setNaturalDurationAsString(crtPlaylistItemScreenSaverVideo._prop_True_Duration, playlistGlobalConfig.getEpsilonTimeBeforeNaturalEnd());
      //---------------------
    ...
    //-------------------------------------------------------------------
    public needToAddFillRect( aPlaylistType: string, jsonObject : any, aPlaylistFile: amGeneralFileInfo.am_general.A_FileInfo,
                              iRealCrtMediaItem : number, iCrtMediaItem : number,
                              aParent: amPlaylistContainer.am_coreservices.AE_PlaylistItem_Container,
                              error: amGeneralError.am_general.A_Error, context: amGeneralContext.am_general.A_Context, callback) : boolean
    {
      /*
      var aParentParent  = aParent.getParent();
      if (aParentParent == null)
        return false;
      if (aParentParent.getPlaylistItemType() == amPlaylistItemType.am_coreservices.AE_PlaylistItemType.PlaylistItemType_DesignMainZone)
      {
        var aParentParentParent  = aParentParent.getParent();
        if (aParentParentParent == null)
          return false;
        var aParentParentParentParent  = aParentParentParent.getParent();
        if (aParentParentParentParent == null)
          return false;
        var aParentParentParentParentParent  = aParentParentParentParent.getParent();
        if (aParentParentParentParentParent == null)
          return false;
        if (aParentParentParentParentParent.getPlaylistItemType() == amPlaylistItemType.am_coreservices.AE_PlaylistItemType.PlaylistItemType_Stream)
        {
          if (jsonObject.Medias_Extended[iCrtMediaItem].MediaType == amJsonMediaTypeId.am_coreservices.AE_JsonMediaTypeId.SEQUENCE_EXTENDED) //Design
          {
            if (jsonObject.Medias_Extended.length == 1)
              return false;
          }
        }*/
      if (jsonObject.Medias_Extended[iCrtMediaItem].MediaType == amJsonMediaTypeId.am_coreservices.AE_JsonMediaTypeId.SEQUENCE_EXTENDED) //Design
      {
        ...
        if (aParentParent == null)
          return false;
        if (aParentParent.getPlaylistItemType() == amPlaylistItemType.am_coreservices.AE_PlaylistItemType.PlaylistItemType_Stream)
        {
          if (jsonObject.Medias_Extended.length == 1)
            return false;
        }
      }else if (jsonObject.Medias_Extended[iCrtMediaItem].MediaType == amJsonMediaTypeId.am_coreservices.AE_JsonMediaTypeId.TAG_EXTENDED) //Tag
      {
        return false;
      }
      return true;
                           ...
                           error: amGeneralError.am_general.A_Error, context: amGeneralContext.am_general.A_Context, callback)
    {
      ...
      for (var iMediaItem = 0; iMediaItem < jsonObject.Medias_Extended.length; iMediaItem++) 
      {
         ...
         {
           if (this.needToAddFillRect(aPlaylistType, jsonObject,  aPlaylistFile,  iRealCrtMediaItem, iMediaItem, aParent, error, context, callback))
           {
    ...
    error: amGeneralError.am_general.A_Error, context: amGeneralContext.am_general.A_Context, callback)
{
    ...
    let _aParent: amPlaylistItem.am_coreservices.AE_PlaylistItem = <amPlaylistItem.am_coreservices.AE_PlaylistItem>aParent;
    do{
      if (_aParent.getPlaylistItemTypeName() === "STREAM") {
        prepareParams.getRenderParams().getParamIdentification().setStreamId(_aParent.getPlaylistItemId());
        break;
      }
      _aParent = _aParent.getParent();
    } while (typeof(_aParent) !== 'undefined' && _aParent);

    prepareParams.getRenderParams().getParamIdentification().setId(jsonObject.ID);
    prepareParams.getRenderParams().getParamIdentification().setName(jsonObject.Name);
                     ...
                     error: amGeneralError.am_general.A_Error, context: amGeneralContext.am_general.A_Context, callback)
{
  ...
  crtPlaylistItemVideoLogic.setNaturalDurationAsString(crtPlaylistItemVideo._prop_True_Duration, playlistGlobalConfig.getEpsilonTimeBeforeNaturalEnd());
  //---------------------
...
{
  //---------- case amJsonMediaTypeId.am_coreservices.AE_JsonMediaTypeId.HTML_EDITABLE_EXTENDED:
  var crtHtmlPage = this._owner._aServiceLocator._iEntityCreation.createDefaultPlaylistItem_HtmlTemplate(error);
  crtHtmlPage.injectDependencies(this._owner._aServiceContainer, this._owner._aServiceLocator, this._owner._aLogService, error);

  crtHtmlPage.setPlaylistItemType(amPlaylistItemType.am_coreservices.AE_PlaylistItemType.PlaylistItemType_HtmlTemplate);
  ...
  crtHtmlPage._main_CustomID              = jsonObject.CustomID;
  crtHtmlPage._main_arrDiffusionDays      = jsonObject.DiffusionDays;
  //crtHtmlPage._main_EndMedia              = jsonObject.EndMedia;
  crtHtmlPage._main_EndTime               = jsonObject.EndTime;
  crtHtmlPage._main_ExpectedPlayDate      = jsonObject.ExpectedPlayDate;
  crtHtmlPage._main_ExpectedPlayTime      = jsonObject.ExpectedPlayTime;
  crtHtmlPage._main_FirstDiffusion        = jsonObject.FirstDiffusion;
  ...
  crtHtmlPage._main_ContentPlayerFileName = jsonObject.ContentPlayerFileName;
  crtHtmlPage._main_Flag                  = jsonObject.Flag;
  //crtHtmlPage._main_RemoteProperties      = jsonObject.RemoteProperties;
  crtHtmlPage._main_RemoteMediaProperties = jsonObject.RemoteMediaProperties;
  crtHtmlPage._main_State                 = jsonObject.State;
  ...
  */
  ...
  //------------ create and set the playlist-item logic
  var crtHtmlPageLogic = this._owner._aServiceLocator._iEntityCreation.createDefaultPlaylistItem_HtmlTemplateLogic(error);
  crtHtmlPageLogic.injectDependencies(this._owner._aServiceContainer, this._owner._aServiceLocator, this._owner._aLogService, error);
  crtHtmlPage.setLogic(crtHtmlPageLogic);
  crtHtmlPageLogic.setOwnerIdxAsChild(iMediaItem);
  crtHtmlPageLogic.setOwnerIdxAsLevel(aParent.getLogic().getOwnerIdxAsLevel()+1);
  ...
  var extraPath =  jsonObject.ContentPlayerFileName;//aCrtHtmlTemplateFileTransfer.getDstFileInfo().getName();
  ...
  var unzipFolderName =  playlistGlobalConfig._aHtmlTemplatesFilesFolder.getUrlPath() + extraPath + "/";

  prepareParams.getRenderParams().getParamFileInfo().setUrlStorage(playlistGlobalConfig._aHtmlTemplatesFilesFolder.getUrlStorage());//"http://127.0.0.1:9080/c/");//aUrlStorage);
  prepareParams.getRenderParams().getParamFileInfo().setUrlPath(unzipFolderName);//playlistGlobalConfig._aHtmlTemplatesFilesFolder.getUrlPath());//"/media_files/");
  prepareParams.getRenderParams().getParamFileInfo().setUrlName("index.html");//"flori01.jpg"); //("WuXi03.png");
 ...
 //---- set html template parameters
 var bIsHtmlTemplate           : boolean = true;
 var sHtmlTemplateUrl          : string  = playlistGlobalConfig._aHtmlTemplatesFilesFolder.getUrlStorage() + unzipFolderName + "index.html";
 var sHtmlTemplateData         : string  = "";  ///encodeURIComponent("file:///opt/media/USBDriveA1/html_templates/" + extraPath + "_"  + jsonObject.ID + ".json");
                                                //"..%2F" + extraPath + "_"  + jsonObject.ID + ".json";//encodeURI("..%2F" + extraPath + "_"  + jsonObject.ID + ".json"); //%5C %2F
 var sHtmlTemplatePlatformType : string  = "";
 if (playlistGlobalConfig._realPlatform == "tizen")
 {
   sHtmlTemplateData     = encodeURIComponent("../" + extraPath + "_"  + jsonObject.ID + ".json");//encodeURIComponent("file:///opt/media/USBDriveA1/html_templates/" + extraPath + "_"  + jsonObject.ID + ".json");
                                               //"..%2F" + extraPath + "_"  + jsonObject.ID + ".json";//encodeURI("..%2F" + extraPath + "_"  + jsonObject.ID + ".json"); //%5C %2F
   sHtmlTemplatePlatformType = "tizen";
 }else{
   sHtmlTemplateData     = encodeURIComponent("../" + extraPath + "_"  + jsonObject.ID + ".json");
   sHtmlTemplatePlatformType = "windows";
 }
 var nHtmlTemplateDuration     : number  = -1;
 if (jsonObject.Duration != null)
   nHtmlTemplateDuration = jsonObject.Duration;
 var bHtmlTemplateAutoplay     : boolean = true;
 var nHtmlTemplatePlayId       : number  = 0;
 prepareParams.getRenderParams().getParamHtmlTemplate().setIsHtmlTemplate(bIsHtmlTemplate);
 prepareParams.getRenderParams().getParamHtmlTemplate().setHtmlTemplateUrl(sHtmlTemplateUrl);
 prepareParams.getRenderParams().getParamHtmlTemplate().setHtmlTemplateData(sHtmlTemplateData);
 prepareParams.getRenderParams().getParamHtmlTemplate().setHtmlTemplatePlatformType(sHtmlTemplatePlatformType);
 prepareParams.getRenderParams().getParamHtmlTemplate().setHtmlTemplateDuration(nHtmlTemplateDuration);
 prepareParams.getRenderParams().getParamHtmlTemplate().setHtmlTemplateAutoplay(bHtmlTemplateAutoplay);
 prepareParams.getRenderParams().getParamHtmlTemplate().setHtmlTemplatePlayId(nHtmlTemplatePlayId);

 var finalUrl = prepareParams.getRenderParams().getParamHtmlTemplate().getHtmTemplateFinalUrl();
 var indexUrl = prepareParams.getRenderParams().getParamHtmlTemplate().getHtmTemplateBasicUrl();
                          ...
                          error: amGeneralError.am_general.A_Error, context: amGeneralContext.am_general.A_Context, callback)
{
  ...
  if (isMainZone)
  {
    ...
    //new
    //crtPlaylistItemDesignZoneLogic.cmd_setNextChildSelectionType(
                             // amNextChildSelectionType.am_coreservices.AE_NextChildSelectionType.NextChildSelectionType_OneAfterTheOther_SecondaryDesignZone);
  }
                          ...
                          error: amGeneralError.am_general.A_Error, context: amGeneralContext.am_general.A_Context, callback)
{
  ...
  crtPlaylistItemVideoStreamLogic.setNaturalDurationAsString(crtPlaylistItemVideoStream._prop_True_Duration, playlistGlobalConfig.getEpsilonTimeBeforeNaturalEnd());
  //---------------------
