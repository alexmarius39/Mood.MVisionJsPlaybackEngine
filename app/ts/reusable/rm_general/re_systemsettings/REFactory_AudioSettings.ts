import amEntity               = require("../../../../../app/ts/abstract/am_general/a_entity/A_Entity");
import amError                = require("../../../../../app/ts/abstract/am_general/a_error/A_Error");
import amFactoryAudioSettings = require("../../../../../app/ts/abstract/am_general/ae_systemsettings/AEFactory_AudioSettings");

import rmAudioSettings = require("./RE_AudioSettings");
import rmEntityFactory = require("../../../../../app/ts/reusable/rm_general/r_entity/RFactory_Entity");

export module rm_general
{
  export class REFactory_AudioSettings  extends    rmEntityFactory.rm_general.RFactory_Entity
                                        implements amFactoryAudioSettings.am_general.AEFactory_AudioSettings
  {
    createEntity(entityName : string, error: amError.am_general.A_Error): amEntity.am_general.A_Entity
    {
      return new rmAudioSettings.rm_general.RE_AudioSettings();
    }
  }
} 

