...
export module nm_transversalservices
{
  export class IImpl_SDK_TvSignageSetup_NodeJs implements amGeneral.am_transversalservices.I_SDK_TvSignageSetup
  {
    ...
    public captureScreen(captureScreenInfo: amGeneralCaptureScreenInfo.am_general.A_CaptureScreenInfo, error: amGeneralError.am_general.A_Error, context: amGeneralContext.am_general.A_Context, callback) : void 
    {
      const self = this;
      let screenshot = null;      
      let aDstFileInfo: amGeneralFileInfo.am_general.A_FileInfo = null;
      let filePath: string = null;
      let screenshotOptions = {};
      let screenshotFormat: string = null;
      let bDifferentFormat: boolean = false;

      try {
        aDstFileInfo = captureScreenInfo.getFileInfo();
        filePath = aDstFileInfo.getFullName();
        const filePathExtension: string = filePath.split('.').pop();

        const screenshotQuality: number = captureScreenInfo.getQuality();
        const screenshotWidth: number   = captureScreenInfo.getWidth();
        const screenshotHeight: number  = captureScreenInfo.getHeight();
      ...
      if (screenshot != null) {
        screenshot(filePath, screenshotOptions, function(err, complete) {
          ...
          if (!error.isError() && bDifferentFormat == true) {
            const cbMoveFile = function(ctx1 : amGeneralContext.am_general.A_Context) {
            ...
            const errorMoveFile:amGeneralError.am_general.A_Error         = self._owner._aServiceLocator._iEntityCreation.createDefaultError();   
            const contextMoveFile:amGeneralContext.am_general.A_Context   = self._owner._aServiceLocator._iEntityCreation.createDefaultContext();  
            contextMoveFile.setRemoteCallback(true);
            self._owner._iSDKFileSystem.moveFile2(
                    aDstFileInfo.getStorage(), aDstFileInfo.getPath(), aDstFileInfo.getName() + `.${screenshotFormat}`, 
                    aDstFileInfo.getStorage(), aDstFileInfo.getPath(), aDstFileInfo.getName(), errorMoveFile, contextMoveFile, cbMoveFile); 
          } else {
            context != null && context.setError(error);
            callback != null && callback(context);
          }
