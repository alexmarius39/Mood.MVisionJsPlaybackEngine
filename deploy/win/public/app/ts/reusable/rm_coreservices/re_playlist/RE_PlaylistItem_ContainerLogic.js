    ...
    (function (rm_coreservices) {
        var RE_PlaylistItem_ContainerLogic = /** @class */ (function (_super) {
            ...
            RE_PlaylistItem_ContainerLogic.prototype.hasTheSameChildNextToPlay_InChannelPlaylist = function (crtChildIdx, event, eventQueue, error, aPlaylistController, aRenderingController, context, callback) {
                ...
                var refDate = this.getSDKGoodCrtDate(); //ma2 //new Date(); 
                var findChild = null;
                while (true) {
            ...
            RE_PlaylistItem_ContainerLogic.prototype.cmd_selectNextChildToPlay = function (crtChildIdx, nextChildSelectionType, event, eventQueue, error, aPlaylistController, aRenderingController, context, callback) {
                switch (nextChildSelectionType) {
                    ...
                    case amNextChildSelectionType.am_coreservices.AE_NextChildSelectionType.NextChildSelectionType_OneAfterTheOther_SecondaryDesignZone:
                        return this.cmd_selectNextChild_OneAfterTheOther_SecondaryDesignZone(crtChildIdx, event, eventQueue, error, aPlaylistController, aRenderingController, context, callback);
                    default:
                        break;
                }
                return null;
            ...
            //======================
            RE_PlaylistItem_ContainerLogic.prototype.cmd_selectNextChild_OneAfterTheOther_SecondaryDesignZone = function (crtChildIdx, event, eventQueue, error, aPlaylistController, aRenderingController, context, callback) {
                var thisId = this.getOwner().getPlaylistItemId();
                var a = 0;
                if (thisId == 68587) {
                    a = 1;
            ...
            //========================================================
            //-------------------------------------
            RE_PlaylistItem_ContainerLogic.prototype.shutdown = function (error, context, callback) {
                ...
                if (nbChildren <= 1)
                    return 1;
                var crtChild = null;
                for (var crtIdx = 0; crtIdx < nbChildren; crtIdx++) {
                    crtChild = childrenList[crtIdx];
                    if (crtChild == null)
                        continue;
                    crtChild.getLogic().shutdown(error, context, callback);
                }
                return 0;
            };
