...
define(["require", "exports", "../../../../../app/ts/reusable/rm_transversalservices/r_utilsservice/asmcryptoes5/entry-export_all", "../../../../../app/ts/reusable/rm_transversalservices/r_sdk_jstv/IImpl_SDK_FileSystem_JsTV_R"], function (require, exports, rmAsmCrypto, rmTransversalServicesFileSystemJsTV) {
    ...
    (function (nm_transversalservices) {
        var IImpl_SDK_FileSystem_NodeJs = /** @class */ (function (_super) {
            ...
            //---------------------
            IImpl_SDK_FileSystem_NodeJs.prototype.readBinaryFile = function (storageName, folderName, fileName, error, context, callback) {
                if (callback == null)
                    return;
                ...
                try {
                    fs = require('fs');
                    fileFullName = aFileInfo.getFullName();
                }
                catch (e) {
                    error != null && error.setError(1001, "[Node.js] readBinaryFile exception " + e.message);
                    context != null && context.setError(error);
                    return callback(context);
                }
                if (fs != null && typeof fs.readFile === "function" && fileFullName != null) {
                    fs.readFile(fileFullName, null, function (err, data) {
                        if (err != null) {
                            error != null && error.setError(1001, "Error reading file " + fileFullName + " - " + err.message);
                        }
                        else {
                            // data is of type ArrayBuffer: https://blog.bitsrc.io/javascript-typed-arrays-ccfa5ae8838d
                            // to access data, we get its' View object
                            self._owner._debug == true && console.log("Successfully read the binary file: " + fileFullName);
                            var bufView = new Uint8Array(data);
                            context != null && context.setArrayResult(Array.from(bufView));
                        }
                        context != null && context.setError(error);
                        callback(context);
                    });
                    ...
                    error != null && error.setError(1011, "[Node.js] error in funcion readBinaryFile");
                    context != null && context.setError(error);
                    callback(context);
                }
            };
            ...
            IImpl_SDK_FileSystem_NodeJs.prototype.downloadFile2 = function (remoteUrlName, dstStorageName, dstFolderName, dstFileName, error, context, callback) {
                ...
                try {
                    ...
                    aFileInfo.setType("file");
                    context.setObjectResult(aFileInfo);
                    //-------------------------------------------------- options // TO DO make parametrization og GET/POST etc.
                    var options = {
                        url: remoteUrlName,
                        method: 'GET',
                        headers: {
                    ...
                    try {
                        ...
                        if (self._owner._aDefaultHttpHeaders != null) {
                            ...
                            if (aHttpHeaderList != null) {
                                ...
                                for (var idxHeader = 0; idxHeader < aHttpHeaderList.length; idxHeader++) {
                                    console.log("    idxHeader: " + idxHeader);
                                    ...
                                    options.headers[crtHttpHeaderName] = crtHttpHeaderValue;
                                    console.log("    Set HTTP HEADER " + crtHttpHeaderName + ":" + options.headers[crtHttpHeaderName]);
                                }
                            }
                        }
                    }
                        ...
                        error.setError(979, "ERROR 579 DOWNLOAD SET HTTP HEADERS: " + JSON.stringify(e.message));
                        console.log("    ERROR 579 DOWNLOAD SET HTTP HEADERS: " + JSON.stringify(e.message));
                        context.setError(error);
                        if (callback != null)
                            callback(context);
                    }
        ...
        fs.createReadStream(inputFileName)
          ...
          */
            ...
            IImpl_SDK_FileSystem_NodeJs.prototype.moveFile2 = function (srcStorageName, srcFolderName, srcFileName, dstStorageName, dstFolderName, dstFileName, error, context, callback) {
                ...
                var srcFileFullName = srcStorageName + srcFolderName + srcFileName;
                var dstFileFullName = dstStorageName + dstFolderName + dstFileName;
                // Async with callbacks: //mm
                fs.move(srcFileFullName, dstFileFullName, { overwrite: true }, function (err) {
                    if (err) {
                        console.error(err);
            ...
            */
            //--------------------------------
            IImpl_SDK_FileSystem_NodeJs.prototype.getLastModified = function (aFileInfo, error, context, callback) {
                if (callback == null)
                    return;
                var self = this;
                var fullName = null;
                ...
                try {
                    fs = require('fs');
                    fullName = aFileInfo.getFullName();
                }
                catch (e) {
                    self._owner._debug == true && console.log("node.js: error getLastModified " + fullName + " exception [ " + e.message + " ]");
                    error != null && error.setError(9089, "node.js: error getLastModified " + fullName + " exception [ " + e.message + " ]");
                    context != null && context.setError(error);
                    return callback(context);
                }
                if (fs != null && typeof fs.stat === "function" && fullName != null) {
                    fs.stat(fullName, function (err, stats) {
                        if (err == null && stats != null &&
                            ((typeof stats.isFile === "function" && stats.isFile()) || (typeof stats.isDirectory === "function" && stats.isDirectory()))) {
                            context.setResult(stats.mtime);
                        }
                        else {
                            var errStr = err ? err.message : "not a file or directory";
                            self._owner._debug == true && console.log("[node.js]: getLastModified error: " + fullName + " stats : error [ " + errStr + " ]");
                            error != null && error.setError(9088, "[node.js]: error getLastModified " + fullName + " stats : error [ " + errStr + " ]");
                        }
                        context != null && context.setError(error);
                        callback(context);
                    });
                    ...
                    error != null && error.setError(9090, "[node.js]: error in function getLastModified");
                    context != null && context.setError(error);
                    callback(context);
                }
            };
